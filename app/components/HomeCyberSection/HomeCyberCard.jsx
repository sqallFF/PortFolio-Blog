...
export default function HomeCyberCard({title, date, media, id}) {
  ...
  return (
    <div className='p-2 min-w-[15rem] md:min-w-[30vw] overflow-hidden' >
    <motion.div className=' relative overflow-hidden w-full h-[55%] sm:h-[65%]' >
      <Link href={`/blogs/${id}`}>
...
<Link href={`/blogs/${id}`} className='py-4 text-lg'>Read More</Link>
<ImArrowRight2/>
</div>
</div>
  )
